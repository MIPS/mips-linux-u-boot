...
static int pch_gbe_mac_write(struct pch_gbe_regs *mac_regs, u8 *addr)
{
	...
	return wait_for_bit_le32(&mac_regs->addr_mask, PCH_GBE_BUSY, false,
				 PCH_GBE_TIMEOUT, false);
}
...
static int pch_gbe_reset(struct udevice *dev)
{
	...
	err = wait_for_bit_le32(&mac_regs->reset, PCH_GBE_ALL_RST, false,
				PCH_GBE_TIMEOUT, false);
	if (err)
		return err;
...
static void pch_gbe_tx_descs_init(struct udevice *dev)
{
	...
	dev_dma_cache_writeback(dev,
	    (ulong)tx_desc, (ulong)&tx_desc[PCH_GBE_DESC_NUM]);
...
static int pch_gbe_send(struct udevice *dev, void *packet, int length)
{
	...
	return wait_for_bit_le32(&mac_regs->int_st, PCH_GBE_INT_TX_CMPLT, true,
				 PCH_GBE_TIMEOUT, false);
}
...
{
	return wait_for_bit_le32(&mac_regs->miim, PCH_GBE_MIIM_OPER_READY,
